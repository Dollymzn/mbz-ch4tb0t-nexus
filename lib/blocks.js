...
const KNOWN_BLOCKS = Object.keys(BLOCK_META);

// ============================================================
//  GUIAS DE ENGENHARIA DE PROMPT POR PLATAFORMA (bloco creatives_prompt)
// ============================================================
const PLATFORM_GUIDES = {
  google_flow: 'REGRAS PARA GOOGLE FLOW / IMAGEFX (Imagen): escreva cada prompt como FRASES NATURAIS E DESCRITIVAS em inglês, nunca lista de palavras-chave. Estrutura de cada prompt: (1) sujeito e ação principal da cena; (2) cenário e atmosfera; (3) câmera e lente explícitas (ex: "shot on 85mm lens, shallow depth of field", "wide angle editorial shot"); (4) iluminação NOMEADA (golden hour, soft studio light, neon glow, candlelight); (5) mood/estilo (cinematic, premium editorial photography); (6) TEXTO NA IMAGEM: o Imagen renderiza texto muito bem — inclua o headline ENTRE ASPAS com instrução explícita de tipografia e posição (ex: with bold headline text "VOCÊ FOI ESCOLHIDA" in elegant serif typography at the top); (7) área limpa reservada para o botão de CTA. PROIBIDO: keyword-spam separado por vírgulas, pesos ::, parâmetros --ar ou --v (não existem no ImageFX).',
...
Responda JSON: {"prompt":"<markdown>"}`;
      ...
      return `${ctx}
Gere ${n} prompts de imagem para a plataforma ${plat} (formato ${sz}) para anuncios do nicho "${p.niche}" no idioma ${p.flowLang || 'en-US'}. Estilo chamativo, gamificado quando possivel, COM texto curto de headline e CTA descrito. Cada prompt pronto pra colar na ferramenta.
${platformGuide}
${gameHookRule}
...
module.exports = { buildBlockPrompt, BLOCK_META, KNOWN_BLOCKS, menuBreakdown, creativesSummary, optimizeIsJsonKind };
