...
A APROVAÇÃO depende do SCORE (>= a nota mínima definida pelo sistema), não do veredito textual.`;
    ...
    case 'creative_analysis': {
      ...
      const niche = ca.niche || p.niche || 'nicho';
      const persona = ca.persona || p.personaLabel || 'sem persona';
      const flowLang = ca.flowLang || p.flowLang || 'en-US';
...
NICHO: ${niche}
PERSONA: ${persona}
IDIOMA DOS TEXTOS (headline/cta): ${flowLang}
${metricsLine}

EXTRAÇÃO DE DNA: identifique mecânica (roleta/presente/raspadinha/etc.), paleta de cor, emoção transmitida, composição e o padrão de headline do criativo vencedor. Levante a hipótese do PORQUÊ venceu (gatilho/público/formato) e liste elementos-chave objetivos. Sinalize riscos de política do Meta na imagem (nudez, before/after, promessa irreal, logos), ou [] se não houver.

GERAÇÃO DE VARIAÇÕES: cada uma das ${n} variações altera UMA ÚNICA dimensão (mecânica, cor, headline, personagem OU composição) em relação ao DNA — A/B estruturado, não ruído aleatório. Cada variação é um ANÚNCIO COMPLETO e pronto pra publicar (headline + botão de CTA renderizados na imagem). Cada "prompt" no formato EXATO da plataforma, seguindo estas regras:
${fullCreativeRules(plat, sz)}
${platformPromptRules(plat, sz)}
"headline" e "cta" no idioma ${flowLang}; o "cta" é o texto EXATO dentro do botão renderizado e o "prompt" DEVE descrever esse botão desenhado.

Responda SOMENTE com este JSON, sem markdown, no schema EXATO:
{"analysis":{"dna":"...","porque_venceu":"...","elementos_chave":["mecânica: ...","cor: ...","headline: ..."],"alertas_meta_policy":["..."]},"variations":[{"index":1,"dimensao_variada":"mecânica → raspadinha","prompt":"...","headline":"...","cta":"..."}]}
Gere EXATAMENTE ${n} variações, index de 1 a ${n}.`;
    }
