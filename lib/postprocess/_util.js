...
// utmNative=true: o ChatDrink tem rastreamento UTM nativo — usa só {{URL_REDIR}}, sem query string.
function fixButtonsUtm(buttons, src, content, utmNative) {
  if (!Array.isArray(buttons)) return;
  buttons.forEach(b => {
    const url = utmNative
      ? '{{URL_REDIR}}'
      : '{{URL_REDIR}}?utm_source=' + src + '&utm_campaign={{UTM_CAMPAIGN}}&utm_medium={{NOMEDAPAGINA}}&utm_term=' + src + '&utm_content=' + content;
