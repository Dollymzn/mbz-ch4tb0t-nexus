...
module.exports = function fixSequenceChatdrink(j, params) {
  ...
  contentIdx.forEach((idx, k) => {
    ...
    route.interactions.forEach(it => {
      if (it.type === 'menu' && it.config) (it.config.cards || []).forEach(c => fixButtonsUtm(c.buttons, 'sequence', 'seq' + num + '-' + slug, utmNative));
  ...
  if (fb && Array.isArray(fb.interactions)) {
    fb.interactions.forEach(it => { if (it.type === 'botoes' && it.config) fixButtonsUtm(it.config.buttons, 'sequence', 'seqf-' + slug, utmNative); });
  }
  return j;
};
